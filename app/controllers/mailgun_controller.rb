            ...
            if u = User.find(:first, from)
                ...
                when /(.+)@lists\.boilerinvasion\.org/i
                    ...
                    if $1 =~ /students/i
                            ...
                            Mailgun.send(Student.list, from, "[#{g.cn.upcase}] #{subject}", body)
                    elsif $1 =~ /mentors/i
                            Rails.logger.info("Sending message to mentors")
                            Mailgun.send(Mentor.list, from, "[#{g.cn.upcase}] #{subject}", body)
                    elsif $1 =~ /parents/i
                            Rails.logger.info("Sending message to parents")
                            Mailgun.send(Parent.list, from, "[#{g.cn.upcase}] #{subject}", body)
                    elsif $1 =~ /everyone/i
                            Rails.logger.info("Sending message to everyone")
                            Mailgun.send(User.list, from, "[#{g.cn.upcase}] #{subject}", body)
                    else
                        if g = Group.find(:first, $1)
                            Rails.logger.info("Sending message to #{g.cn.upcase}")
                            Mailgun.send(g.list, from, "[#{g.cn.upcase}] #{subject}", body)
                        else
