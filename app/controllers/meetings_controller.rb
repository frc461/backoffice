class MeetingsController < ApplicationController
  before_action :set_meeting, only: [:show, :edit, :update, :destroy, :attend, :unattend, :list]

  # GET /meetings
  # GET /meetings.json
  def index
    if params[:scale] == 'all'
        @meetings = Meeting.all
    elsif params[:start] || params[:finish]
        @meetings = Meeting.where('start > ? AND finish < ?', DateTime.parse(params[:start]).beginning_of_day, DateTime.parse([params[:finish]]).end_of_day)
    else
    @meetings = Meeting.where('start > ?', DateTime.now.beginning_of_day).first(5)
    end
  end

  # GET /meetings/1
  # GET /meetings/1.json
  def show
  end

  # GET /meetings/new
  def new
    @meeting = Meeting.new
  end

  # GET /meetings/1/edit
  def edit
  end

  def list
  end

  # POST /meetings
  # POST /meetings.json
  def create
    @meeting = Meeting.new(meeting_params)

    respond_to do |format|
      if @meeting.save
        format.html { redirect_to @meeting, notice: 'Meeting was successfully created.' }
        format.json { render :show, status: :created, location: @meeting }
      else
        format.html { render :new }
        format.json { render json: @meeting.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /meetings/1
  # PATCH/PUT /meetings/1.json
  def update
    respond_to do |format|
      if @meeting.update(meeting_params)
        format.html { redirect_to @meeting, notice: 'Meeting was successfully updated.' }
        format.json { render :show, status: :ok, location: @meeting }
      else
        format.html { render :edit }
        format.json { render json: @meeting.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /meetings/1
  # DELETE /meetings/1.json
  def destroy
    @meeting.destroy
    respond_to do |format|
      format.html { redirect_to meetings_url, notice: 'Meeting was successfully destroyed.' }
      format.json { head :no_content }
    end
  end

  def attend
      if @meeting.verified
          if current_mentor || current_role?('attendance') || current_role?('exec')
              Checkin.create(:meeting_id => @meeting.id, :user_dn => params[:user_dn])
              q = "Checked in."
          else
              q = "An approved user must check you in to this meeting."
          end
      else
          if current_user.dn == params[:user_dn] || current_mentor || current_role?('attendance') || current_role?('exec')
              Checkin.create(:meeting_id => @meeting.id, :user_dn => params[:user_dn], notes: params[:notes])
              q = "Checked in."
          else
              q = "Only approved users can check other people into meetings."
          end
      end

      redirect_to @meeting
  end

  def unattend
      if @meeting.verified
          if current_mentor || current_role?('attendance') || current_role?('exec')
              Checkin.where(:meeting_id => @meeting.id, :user_dn => params[:user_dn]).delete_all
              q = "Left meeting."
          else
              q = "An approved user must remove you in to this meeting."
          end
      else
          if current_user.dn == params[:user_dn] || current_mentor || current_role?('attendance') || current_role?('exec')
              Checkin.where(:meeting_id => @meeting.id, :user_dn => params[:user_dn]).delete_all
              q = "Left meeting."
          else
              q = "Only approved users can remove other people from meetings."
          end
      end
      redirect_to @meeting
  end

  private
    # Use callbacks to share common setup or constraints between actions.
    def set_meeting
      @meeting = Meeting.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def meeting_params
      params.require(:meeting).permit(:name, :description, :location, :start, :finish, :required, :verified)
    end
end
